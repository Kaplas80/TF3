namespace TF3.Tests.Converters.BitmapImage
{
    ...
    public class WriterTests
    {
        ...
        public void InvalidFormatThrowsException()
        {
            ...
            converter.Initialize(new ImageParameters { PixelFormat = (BitmapPixelFormat)1000 });
            _ = Assert.Throws<InvalidOperationException>(() => converter.Convert(bmp));
        }
        ...
        {
            var converter = new Writer();
            converter.Initialize(new ImageParameters { PixelFormat = BitmapPixelFormat.Rgba32, ImageWidth = 1, ImageHeight = 1 });
            var format = new BitmapFileFormat();
            _ = Assert.Throws<NullReferenceException>(() => converter.Convert(format));
        }

        [Test]
        public void WriteImage([Values] BitmapPixelFormat pixelFormat)
        {
            ...
            Assert.IsTrue(result.Stream.Compare(ds));
        }
    }
}
